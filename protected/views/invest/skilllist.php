	...
	<tbody role="alert" aria-live="polite" aria-relevant="all">
		...
		foreach($projectArrs as $row) {
			echo '<tr class="odd">'
					...
					.'<td class="">'.$row['recommend'].'</td>'
					.'<td class="">'.$model->getStatus($row['status']).'</td>'
					.'<td class=""><ul class="list-unstyled" >'
					.'<li><a class="btn btn-primary btn-xs" target="_blank" href="'.$this->createUrl('skillupdate', array('id' => $row['id'])).'">编辑</a></li>'
					.'<li><a class="btn btn-primary btn-xs" target="_blank" href="'.$this->createUrl('skillview', array('id' => $row['id'])).'">查看</a></li>'
					.'</ul></td></tr>';
		} ?>
	</tbody>
	</table>
