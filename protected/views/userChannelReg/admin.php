...
<div class="portlet box blue" >
	...
	<table class="table  table-hover table-bordered dataTable"  aria-describedby="sample_editable_1_info">
		...
		<label >开始</label>
			<div class="col-md-10">
				<?php echo $form->textField($models,'starttime', array('class' => 'form-control  _datePicker','placeholder' =>date("Y-m-d",strtotime("-7 day")))); ?> 
				<?php echo $form->error($models,'starttime', array('class'=>'help-block' )); ?>
			</div>
		</div>
		<div class="form-group">
			<label>结束</label>
			<div class="col-md-10">
				<?php echo $form->textField($models,'endtime', array('class' => 'form-control _datePicker','placeholder' =>date("Y-m-d",strtotime("-1 day")))); ?>
				<?php echo $form->error($models,'endtime', array('class'=>'help-block' )); ?>
			</div>
		</div>
		<div class="">
			...
			<td rowspan='2'>
			<?php foreach($upType as $k=>$v):?>
				<?php foreach($v as $key=>$val):?>
					<div style="width:100px;float:left;"><input type="checkbox" name="search[]"  value="<?php echo $val;?>"/><?php echo $val?></div>
				<?php endforeach;?>
			<?php endforeach;?>
				</td>
		</tr>
			...
			<?php foreach($model as $k=>$v):?>
				<tr><td style="height:30px;"><?php echo $k;?></td>
					<?php for($i=0;$i<count($array);$i++):?>
						<td style="height:30px;"><?php echo $v[$array[$i]];?></td>
					<?php endfor;?>
				</tr>
			<?php endforeach;?>
		<?php else:?>
			<tr><td style='height:30px;'>时间</td>	
		<?php foreach($upType as $key=>$val):?>
			<?php foreach($val as $ke=>$va):?>
				<td style="height:30px";><?php echo $va;?></td>
			<?php endforeach;?>
		<?php endforeach;?>
			<tr>
		<?php endif;?>
